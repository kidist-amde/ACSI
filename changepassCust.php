    ...
    <!-- END of ACSI_menubar -->
	  ...
	  ?>
	     <div id="content" class="float_l"> <br/>
	  		 ...
	  		 <a href=manageAccCust.php ><img src="images/up.png" width="22" height="25" title=Back align='top'/></a><br/>

		<br/><br/><div align='center' style="border-radius: 5px;
				border:1px double #b9b9b9;width:680"><br/>
			<form action="changepassCust.php" method="post" name="myform">
		...
		<?php
				...
				$result = mysql_query("SELECT * FROM customer WHERE password='{$cpass}' AND AccNumber='{$AccNumber}' ");
				if(!$result){
				die("query faile".mysql_error());
				}
				if(mysql_num_rows($result)==1){
				if($npass==$npass1){
				$query="update customer set password='{$password}' where password='{$cpass}' AND AccNumber='{$AccNumber}'";
				$result=mysql_query($query);
				echo '<img src="images/492.png" /> &nbsp;! Your password changed successfully';
				echo '<meta content="3;manageAccCust.php" http-equiv="refresh" />';

					}
				else{
				echo '<img src="images/no.ico" /> &nbsp;! New password and Repeat password is not much';
				echo '<meta content="3;changepassCust.php" http-equiv="refresh" />';
					}
