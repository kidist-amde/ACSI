    ...
    <!-- END of ACSI_menubar -->
	  ...
	  ?>
       ...
       <a href=manageAccCust.php ><img src="images/up.png" width="22" height="25" title=Back align='top'/></a><br/>
		...
		<form  action="changeunCust.php" method="post" name="myform">
		...
		<?php
				if (isset($_POST['change']))
			  {
				$cun =$_POST['curun'];
				$nun =$_POST['newun'];
				$result = mysql_query("SELECT * FROM customer WHERE userName='{$cun}' AND AccNumber='{$AccNumber}' ");
				if(!$result){
				die("query faile".mysql_error());
				}
				if(mysql_num_rows($result)==1){

				$query="update customer set userName = '{$nun}' where userName = '{$cun}' AND AccNumber='{$AccNumber}'";
				$result=mysql_query($query);
				echo '<img src="images/492.png" /> &nbsp;! Your UserName changed successfully';
				echo '<meta content="2;manageAccCust.php" http-equiv="refresh" />';
				...
				echo '<meta content="2;changeunCust.php" http-equiv="refresh" />';
