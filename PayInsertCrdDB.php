...
echo "<center><font color='red' size='3'>You have to enter only number in amount text box</font></center>";
	...
	while ($db_field = mysql_fetch_assoc($result)) {
		...
		$b = $db_field['Amount'];
		if(($acc == $a) AND ($return<=$b))
		   {
		        $e="update credit set Amount=Amount-'$return' where AccNumber='$acc'";
				$yt=mysql_query($e);
			    $ue="INSERT INTO creditret values('$acc','$return','$date')";
				
				$ree=mysql_query($ue);
				echo "<font color='yellow' size='3'>Transaction Has Been performed  successfully</font>";
				echo' <meta content="1;CollectCredit.php" http-equiv="refresh" />';
			}
			
			else
			{
			echo "<font color='yellow' size='3'>You have no that much loan.Please, enter the correct information</font>";
			echo' <meta content="2;CollectCredit.php" http-equiv="refresh" />';
			}
			}}
